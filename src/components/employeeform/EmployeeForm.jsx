...
const EmployeeForm = () => {
    ...
    const [employeeStorage, setEmployeeStorage] = useState(GetStorage());
    ...
    const FormSubmit = () => {
        ...
        const newProduct = {...inputfield,id: Math.floor(Math.random() * 10000)};
        const updatedProducts = [...employeeStorage, newProduct];

        setEmployeeStorage(updatedProducts);
        localStorage.setItem("EmployeeView", JSON.stringify(updatedProducts));
    }

    useEffect(() => {
        localStorage.setItem("EmployeeView", JSON.stringify(employeeStorage));
    }, [employeeStorage])
                        ...
                        <div className="col-12 flex items-center justify-center h-screen">
                            <div className="col-lg-6">
                                <div className="bg-white rounded-lg p-3 shadow-lg border overflow-x-scroll">
                                    <h2 className="text-2xl font-bold text-center text-white bg-blue-700 py-2 rounded-lg mb-3">Employee Form</h2>
                                    ...
                                    <form action="">
                                        ...
                                        <div className="submit-btn text-center mb-1">
                                            <Link to="/employeeView" onClick={FormSubmit} className="bg-blue-500 hover:bg-blue-600 text-white font-medium rounded-lg px-3 py-2 no-underline"> Submit</Link>
                                        </div>
                                    </form>
                                </div>
                            </div>
